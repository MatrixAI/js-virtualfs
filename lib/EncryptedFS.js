...
export default class EncryptedFS {
	...
	*/
	...
	writeSync(fd: number, buffer: Buffer, offset: number, length: number, position: number) {
		...
		const writeOffset = position & (this._blockSize-1); // how far from a block boundary our write is
		const numBlocksToWrite = Math.ceil((length + writeOffset)/ this._blockSize);


		const startBlockNum = this._offsetToBlockNum(position) 
		const endBlockNum = startBlockNum + numBlocksToWrite - 1;
		
		const startBlockOffset = this._blockNumToOffset(startBlockNum);
		...
		//if (this._hasContentSync(fd)) {
			...
			const startBlockOverlaySize = this._blockSize - writeOffset;
			...
			firstBlock = this._overlaySegment(fd, startBlockOverlay, position);
			// only bother if there is a last chunk
			let endBlockBufferOffset;
			if (numBlocksToWrite >= 2) {
				// where the end block begins in the buffer
				endBlockBufferOffset = ((numBlocksToWrite-1)*this._blockSize) + writeOffset;
			...
			firstBlock = buffer.slice(offset, offset + Math.min(length, this._blockSize));
	...
	//	}
		...
		const newBlocks = Buffer.concat([firstBlock, middleBlocks, endBlock]);
		...
		// TODO: flush? 
		fs.writeSync(fd, encryptedWriteBuffer, 0, encryptedWriteBuffer.length, startBlockOffset);

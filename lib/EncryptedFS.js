...
export default class EncryptedFS {
	...
	*/
	...
	readSync(fd: number, buffer: Buffer, offset: number, length: number, position: number) {
		...
		// i.e. blockNum and chunkNum can be used interchangably 
	...
	_readBlock(fd: number, position: number) {
		const blockNum = this._offsetToBlockNum(position);
		const blockStart = blockNum * this._blockSize;
		const blockBuf = Buffer.allocUnsafe(this._blockSize);

		this.readSync(fd, blockBuf, 0, this._blockSize, blockStart);
	...
	_overlaySegment(fd: number, newData: Buffer, position: number) {
		...
		const startBlockAligned = ((position & this._blockSize-1) === 0) 
		if (!(startBlockAligned)) {
			startSlice = origBlock.slice(0, position-1);
		}

		// any data reamining after new block
		const endSlice = origBlock.slice(position + newData.length)
	...
	_makeBlockIterable(buffer: Buffer) {
			...
			return {
				next: () => {
					...
					if (currOffset <= buffer.length) {
						result = { 
							value: buffer.slice(currOffset, currOffset + this._blockSize - 1),
							done: false 
						}
		...
		console.log(buffer[Symbol.iterator]())
		return buffer;
	}
	...
	writeSync(fd: number, buffer: Buffer, length: number, offset: number, position: number) {
		...
		if (!(length)) {
			console.log("write(): 'length' not defined, using buffer.length: " + buffer.length);
			length = buffer.length;
		}
		
		const numBlocksToWrite = Math.ceil(length / this._blockSize);

		const overlayBufSize = (this._blockSize-1) - (position & this._blockSize-1) 
		const firstBlockOverlay = buffer.slice(position, position + overlayBufSize)

		// TODO: first and last block should really be called start and end block
		const firstBlockNum = this._offsetToBlockNum(position + length) 
		const lastBlockNum = firstBlockNum + numBlocksToWrite - 1;
		
		const firstBlockOffset = this._blockNumToOffset(firstBlockNum);
		const lastBlockOffset = this._blockNumToOffset(lastBlockNum);


		// only the first and last block needs to be overlayed
		// they needs to be process separately first
		const firstBlock = this._overlaySegment(fd, firstBlockOverlay, position);
		// alloc(0) in the case we are writing only 1 block
		let lastBlock = Buffer.alloc(0);
		// only bother if there is a last chunk
		if (numBlocksToWrite >= 2) {
			// TODO: length maybe undefined
			const lastBlockOverlay =  buffer.slice(lastBlockOffset)


			lastBlock = this._overlaySegment(fd, lastBlockOverlay, lastBlockOffset)
			
		}
		
		// from start of second block to the end of the second last block
		const middleBlocks = buffer.slice(this._blockNumToOffset(firstBlockNum + 1), lastBlockOffset - 1);

		// TODO: assert newBlocks is a multiple of blocksize 
		const newBlocks = Buffer.concat([firstBlock, middleBlocks, lastBlock]);
		...
		// TODO: flush? 
		fs.writeSync(fd, encryptedWriteBuffer, 0, encryptedWriteBuffer.length, firstBlockOffset);

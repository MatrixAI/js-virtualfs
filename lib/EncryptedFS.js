...
export default class EncryptedFS {
	...
	*/
	...
	// TODO: all of the params are actually required
	...
	writeSync(fd: number, buffer: Buffer, offset: number, length: number, position: number) {
		...
		const firstBlockNum = this._offsetToBlockNum(position) 
		...
		if (this._hasContentSync(fd)) {
			...
			firstBlock = buffer.slice(0, this._blockSize);
			...
			if (firstBlock.length < this._blockSize) {
				firstBlock = Buffer.concat([firstBlock], this._blockSize);
			}

			// always a last block when writing at least 2 blocks
			if (numBlocksToWrite >= 2) {
				lastBlock = buffer.slice(lastBlockOffset);
			}
		}
		...
		if (numBlocksToWrite >= 3) {
			...
			middleBlocks = buffer.slice(this._blockNumToOffset(firstBlockNum + 1), lastBlockOffset - 1);
		}
